...
static void note_sret_return_var(SretReturnVar *retvar, Obj *var) {
  if (!var || retvar->invalid)
    return;
  if (retvar->var && retvar->var != var) {
    retvar->invalid = true;
    return;
  }
  retvar->var = var;
...
static void collect_sret_return_var(Node *node, Type *rty, SretReturnVar *retvar) {
  if (!node || retvar->invalid)
    return;

  switch (node->kind) {
  case ND_RETURN:
    if (!node->lhs || node->lhs->kind != ND_VAR ||
        !node->lhs->var->is_local || !is_compatible(node->lhs->ty, rty)) {
      retvar->invalid = true;
      return;
    }
    note_sret_return_var(retvar, node->lhs->var);
    return;
  case ND_BLOCK:
    for (Node *n = node->body; n; n = n->next)
      collect_sret_return_var(n, rty, retvar);
    return;
  case ND_IF:
    collect_sret_return_var(node->then, rty, retvar);
    collect_sret_return_var(node->els, rty, retvar);
    return;
  case ND_FOR:
  case ND_DO:
  case ND_SWITCH:
  ...
  case ND_CASE:
  case ND_LABEL:
    collect_sret_return_var(node->lhs, rty, retvar);
    return;
  default:
    return;
  }
...
static void mark_sret_alias_candidate(Obj *fn) {
  ...
  collect_sret_return_var(fn->body, rty, &retvar);
  if (!retvar.invalid && retvar.var)
    retvar.var->is_sret_alias = true;
}
...
//          | "asm" "!" "(" ident ("," ident)* ")" "{" string-literal "}"
static Node *asm_stmt(Token **rest, Token *tok) {
  Node *node = new_node(ND_ASM, tok);
  tok = tok->next;

  if (equal(tok, "!")) {
    static char *regs[] = {"4x", "5x", "6x", "7x", "8x", "9x", "ax", "bx"};
    AsmBinding head = {};
    AsmBinding *cur = &head;
    int reg_idx = 0;
    ...
    tok = skip(tok->next, "(");
    while (!equal(tok, ")")) {
      if (reg_idx == sizeof(regs) / sizeof(*regs))
        error_tok(tok, "too many asm! bindings");
      if (tok->kind != TK_IDENT)
        error_tok(tok, "expected an identifier");

      VarScope *sc = find_var(tok);
      if (!sc || !sc->var)
        error_tok(tok, "unknown asm! binding variable");

      AsmBinding *binding = calloc(1, sizeof(AsmBinding));
      binding->name = get_ident(tok);
      binding->reg = regs[reg_idx++];
      binding->var = sc->var;
      cur = cur->next = binding;
    ...
    node->asm_bindings = head.next;
    tok = skip(tok->next, "}");
    *rest = consume(&tok, tok, ";") ? tok : tok;
    return node;
  }

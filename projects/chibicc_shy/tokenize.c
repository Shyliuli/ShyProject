...
static void convert_pp_number(Token *tok) {
  ...
  error_tok(tok, "floating-point constants are not supported by chibicc_shy yet");
}
...
static char *read_file(char *path) {
  ...
  size_t len = 0;
  size_t line_start = 0;
  char *sentinel = "__SHYCC_END__";
  size_t sentinel_len = strlen(sentinel);
  ...
  for (;;) {
    ...
    if (c == '\n') {
      ...
      if (line_len && buf[len - 1] == '\n')
        line_len--;
      if (line_len && buf[line_start + line_len - 1] == '\r')
        line_len--;
      if (line_len == sentinel_len &&
          !memcmp(buf + line_start, sentinel, sentinel_len)) {
        len = line_start;
        break;
      }
      line_start = len;
